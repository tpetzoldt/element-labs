...
We can check the shape of distribution graphically. We do it for now only for the "box1" subset, because it contains more data.
...
```{r }
box1.pi <- box1.mean + c(-1.96, 1.96) * box1.sd
box1.pi
```
...
```{r }
box1.pi <- box1.mean + qnorm(c(0.025, 0.975)) * box1.sd
box1.pi
```
...
```{r }
plot(box1$weight)
abline(h = box1.pi, col="forestgreen")
```
...
```{r }
hist(box1$weight)
abline(v = box1.pi, col="forestgreen")
...
**Task:** Calculate the confidence interval of the mean value of the "box1" data set using the quantile function (`qt`) of the t-distribution^[as the sample size is not too small, you may also compare this with 1.96 or 2.0]:
...
```{r eval = FALSE}
hist(box1$weight, probability = TRUE, ylim = c(0, 0.25))
xnew <- seq(50, 100, length = 500)
lines(xnew, dnorm(xnew, box1.mean, box1.sd), col = "forestgreen")
lines(xnew, dnorm(xnew, box1.mean, box1.se), col = "magenta")
abline(v = box1.pi, col="forestgreen")
abline(v = box1.ci, col="magenta")
```
